...
-added by yhboo-
stacking fixed updates, fixed params added
...
class SimpleGraph(object):
    ...
    def __init__(self, name=None):
        ...
        # set members
        self.name = name
        self.params_float = []  # collection of layer parameters
        self.params_fixed = []
    ...
    def get_params_float(self):
        ...
        """
        return self.params_float

    def get_params_fixed(self):
        return self.params_fixed
    ...
    def add_layer(self, layer):
        ...
        layer_params_fixed = layer.get_params_fixed()
        layer_params_float = layer.get_params_float()
        layer_updates = layer.get_updates()
        layer_fixed_updates = layer.get_fixed_updates()
        layer_output = layer.get_output(self.get_output())
        self.layers.append(layer)
        self.outputs.append(layer_output)
        self.params_float = self.params_float + layer_params_float
        self.params_fixed = self.params_fixed + layer_params_fixed
        self.updates = merge_dicts([self.updates, layer_updates])
        self.fixed_updates = merge_dicts([self.fixed_updates, layer_fixed_updates]) 
    ...
    def show_fixed_params(self):
        for pp in self.layers:
            if type(pp).__name__ == 'FixedDenseLayer' :
                step_size = pp.step_size_w.get_value()
                print('float weight [0:5,0:5]')
                print(pp.W.get_value()[0:5,0:5])
                print('fixed weight [0:5,0:5]')
                print(pp.W_fixed.get_value()[0:10,0:10] / step_size)
                print('step_size')
                print(step_size)