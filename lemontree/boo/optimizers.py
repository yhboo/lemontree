...
class BaseOptimizer(object):
    ...
    '''
    ##added
    def get_updates(self, loss, params_fixed, params_float):
        """
        This function returns computed update rule.
        Actual computation is done is "compute_gradients" and "gradients_to_updates".
        ...
        """
        # check asserts 
        assert isinstance(params_fixed, list), '"params_fixed" should be a list type.'
        assert isinstance(params_float, list), '"params_float" should be a list type.'

        # do
        include_tags_fixed = print_tags_in_params(params_fixed, False)  # include all tags to include all parameters
        include_tags_float = print_tags_in_params(params_float, False)  # include all tags to include all parameters
        unblocked_params_fixed = filter_params_by_tags(params_fixed, include_tags_fixed, self.exclude_tags)
        unblocked_params_float = filter_params_by_tags(params_float, include_tags_float, self.exclude_tags)
        grads = self.compute_gradients(loss, unblocked_params_fixed)
        updates = self.gradients_to_updates(unblocked_params_float, grads)
        return updates
